"""AsyncTasksMixin — torrent tasks (add, progress, pause/resume/delete)."""
...
class AsyncTasksMixin:
    async def list_tasks(self: AsyncClientProtocol):
        """GET /tasks. Requires `tasks.read`."""
        return models.TaskList.model_validate(await self._request("get", "/tasks"))

    async def get_task(self: AsyncClientProtocol, task_id: Union[int, str]):
        """GET /tasks/{id}. Requires `tasks.read`."""
        return models.TorrentTask.model_validate(
            await self._request("get", f"/tasks/{task_id}")
        )

    async def get_task_contents(self: AsyncClientProtocol, task_id: Union[int, str]):
        """GET /tasks/{id}/contents. Requires `tasks.read`."""
        return models.FolderContents.model_validate(
            await self._request("get", f"/tasks/{task_id}/contents")
        )

    async def get_task_progress(self: AsyncClientProtocol, task_id: Union[int, str]):
        """GET /tasks/{id}/progress. Requires `tasks.read`."""
        return models.TaskProgress.model_validate(
            await self._request("get", f"/tasks/{task_id}/progress")
        )

    async def get_task_unwanted(self: AsyncClientProtocol, task_id: Union[int, str]):
        """GET /tasks/{id}/unwanted — indexes marked as unwanted."""
        return models.UnwantedEntries.model_validate(
            await self._request("get", f"/tasks/{task_id}/unwanted")
        )
    ...
    async def set_task_unwanted(
        self: AsyncClientProtocol, task_id: Union[int, str], unwanted: List[int]
    ):
    ...
    async def pause_task(self: AsyncClientProtocol, task_id: Union[int, str]):
        """POST /tasks/{id}/pause. Requires `tasks.write`."""
        data = await self._request("post", f"/tasks/{task_id}/pause")
        return models.APIResult.model_validate(data or {})

    async def resume_task(self: AsyncClientProtocol, task_id: Union[int, str]):
        """POST /tasks/{id}/resume. Requires `tasks.write`."""
        data = await self._request("post", f"/tasks/{task_id}/resume")
        return models.APIResult.model_validate(data or {})

    async def delete_task(self: AsyncClientProtocol, task_id: Union[int, str]):

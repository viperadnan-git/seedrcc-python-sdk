"""AsyncCookieSession — shared cookie login for PAT and device management.
...
async def cookie_login(client: httpx.AsyncClient, username: str, password: str) -> None:
    ...
    response = await make_http_request(
        client,
        "post",
        constants.COOKIE_LOGIN_URL,
        json=payload,
        headers=constants.BROWSER_HEADERS,
    )
...
class AsyncCookieSession:
    ...
    def __init__(
        ...
        *,
        ...
        if not cookies and not (username and password) and httpx_client is None:
            raise AuthenticationError(
                "AsyncCookieSession requires one of: cookies, "
    ...
    async def __aenter__(self) -> "AsyncCookieSession":
    ...
    async def _ensure_logged_in(self) -> None:
        ...
        if self._cookies:
            ...
            await cookie_login(self._client, self._username, self._password)
        elif not self._client.cookies:
            raise AuthenticationError(
                "AsyncCookieSession received only httpx_client but it has no cookies."
            )
    ...
    async def list_pats(self) -> List[models.PersonalAccessToken]:
        """GET /api/v0.1/account/pats."""
        await self._ensure_csrf()
        response = await make_http_request(
            self._client, "get", constants.PATS_URL, headers=self._api_headers()
        )
        if not response.is_success:
    ...
    async def run_device_flow(
        ...
        `AsyncAuthMixin.get_device_code` / `AsyncAuthMixin._poll_device_token`.
        ...
        dc = await AsyncAuthMixin.get_device_code(
            client_id, scope, httpx_client=self._client
        )

        # Step 2: fetch verify page, scrape the form CSRF
        verify_url = f"{constants.DEVICE_VERIFY_URL}?code={dc.user_code}"
        page = await make_http_request(
            self._client, "get", verify_url, headers=constants.BROWSER_HEADERS
        )
        ...
        # Step 3: POST approve
        approve_resp = await make_http_request(
            self._client,
            "post",
            constants.DEVICE_VERIFY_URL,
            data={
                "code": dc.user_code,
                "_csrf_token": match.group(1),
                "action": "approve",
            },
            headers={**constants.BROWSER_HEADERS, "referer": verify_url},
        ...
        return await AsyncAuthMixin._poll_device_token(
            self._client,
            dc.device_code,
            client_id,
            poll=False,
    ...
    async def accept_terms(self, newsletter: bool = False) -> models.TermsAcceptance:
    ...
    async def _fetch_devices_html(self) -> str:
        await self._ensure_logged_in()
        response = await make_http_request(
            self._client,
            "get",
            constants.DEVICES_CONSOLE_URL,
            headers=constants.BROWSER_HEADERS,
        )
        if not response.is_success:
            raise AuthenticationError(
    ...
    async def list_devices(self) -> List[models.AuthorizedDevice]:
        """Scrape the authorized-devices console page into `AuthorizedDevice`s."""
        html = await self._fetch_devices_html()
        return [
    ...
    async def revoke_device(self, device_code: str) -> None:
        ...
        response = await make_http_request(
            self._client,
            "post",
            constants.DEVICE_REVOKE_URL_TEMPLATE.format(device_code=device_code),

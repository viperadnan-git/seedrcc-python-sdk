...
from ._token_handlers import AsyncFileTokenHandler, AsyncTokenHandler
...
class AsyncBaseClient:
    ...
    def __init__(
        ...
        token_handler: Optional[AsyncTokenHandler] = None,
        ...
        **httpx_kwargs: Any,
        ...
        Args:
            token: Optional explicit Token. If omitted, the handler is consulted
                on context-manager entry (`async with ...`).
            token_handler: Persistence backend. Defaults to `AsyncFileTokenHandler()`
                (JSON at `./.cache/seedr_token.json`). Pass `AsyncMemoryTokenHandler()`
                for in-process only.
        ...
        """
        self._handler: AsyncTokenHandler = token_handler or AsyncFileTokenHandler()
    ...
    @property
    def token_handler(self) -> AsyncTokenHandler:
    ...
    async def _refresh_access_token(self) -> None:
        ...
        await self._handler.save(self._token)
    ...
    async def __aenter__(self: TBase) -> TBase:
        if not self._token.access_token and not self._token.refresh_token:
            loaded = await self._handler.load()
